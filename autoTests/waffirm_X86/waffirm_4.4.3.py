...
#
#*******************************************************************************
# waffirm_4.4.3.py - test case 4.4.3 of waffirm_new
#
# Author:  (jinpfb)
#
# Version 1.0.0
...
#
# Features:
# 4.4.3	本地转发模式下对radio下的所有同vap用户实现隔离
# 测试目的：本地转发模式下对radio下的所有同vap用户实现隔离
# 测试环境：见测试拓扑
# 测试描述：本地转发模式下，通过profile->radio配置隔离功能，可以让该 radio下
# 所有同vap无线用户实现二层隔离，不同vap之间不能隔离
#
#*******************************************************************************
# Change log:
#     - 
...
testname = 'TestCase 4.4.3'
printTimer(testname,'Start','test isolation to station of the same \
                             vap in local switch mode')
...
#配置s3p3,s3p4为Trunk 口，allowed vlan 4092，native vlan为20
...
#Step 6 
# 操作
# 在ac1上通过 proflile->radio->命令station-isolation，设置二层隔离，并下发配置
#
# 预期
# Show wireless ap profile 2 radio 1显示
# 有：Station Isolation................ Enable
# 配置下发成功，AP1的配置状态为success
################################################################################
printStep(testname,"Step 6',\
          'Config station-isolation on profile',\
...
data2 = SetCmd(switch1,'show wireless ap profile 2 radio 1')
...
printStep(testname,'Step 7',\
          'STA1 and STA2 cannot ping each other')
...
#Step 10
# 操作
# 在ac1上通过 proflile->radio->命令station-isolation，关闭二层隔离，并下发配置
#
# 预期
# Show wireless ap profile 2 radio 1显示
# 有：Station Isolation................ Enable
...
#Step 11
#操作
#Sta1,sta2下线后都关联AP2的test1，在sta1上ping sta2
#
#预期
#能够 ping 通
################################################################################
