...
#Global Definition
CMD_Set_acl_100_deny_icmp_net92_to_host10 = 'access-list 100 deny icmp ' + Dhcp_pool2 + '0' + ' 0.0.0.255 host-destination '+ pc1_ipv4
CMD_Set_acl_101_deny_icmp_host10_to_net92 = 'access-list 101 deny icmp host-source ' + pc1_ipv4 + ' ' + Dhcp_pool2 + '0' + ' 0.0.0.255'
# CMD_Set_acl_100_permit_icmp_any_to_any = 'access-list 100 permit icmp any-source any-destination'
# CMD_Set_acl_101_permit_icmp_any_to_any = 'access-list 101 permit icmp any-source any-destination '
#Source files
...
printStep(testname,'Step 1',\
          'set ip acl 100 deny icmp source 192.168.92.0 0.0.0.255 host-destination 25.1.1.1192.168.10.2,',\
          'access-list 100 permit any-source any-destination,',\
          'access-list 101 deny icmp host-source 192.168.10.2 destination 192.168.92.0 0.0.0.255,',\
          'access-list 101 permit any-source any-destination,',\
          'config success.')
...
# 
# ##配置静态arp
# static_ip = '100.1.1.31'
# static_mac = '00-00-00-00-00-01'
# CMD_Add_static_arp_s1p5 = 'arp ' + static_ip + ' ' + static_mac +' interface '+ s1p5
# CMD_Delete_static_arp_s1p5 = 'no arp ' + static_ip
# CMD_Ping_sta1_to_static = 'ping '+static_ip
# CMD_Ping_sta1_to_pc = 'ping '+ Var_host_ipaddress
...
SetCmd(switch1,CMD_Set_acl_100_deny_icmp_net92_to_host10)
SetCmd(switch1,CMD_Set_acl_101_deny_icmp_host10_to_net92)
...
res1 = CheckLineInOrder(data1,['access-list 100',\
                               'deny icmp '+ Dhcp_pool2 + '0' +' 0.0.0.255 host-destination '+ pc1_ipv4, \
                               'permit ip any-source any-destination'],IC=True)
res2 = CheckLineInOrder(data1,['access-list 101',\
                               'deny icmp host-source ' + pc1_ipv4 + ' ' + Dhcp_pool2 + '0' + ' 0.0.0.255',\
                               'permit ip any-source any-destination'],IC=True)
...
#
#预期
#STA1关联成功，获取192.168.92.X网段的IP地址。
################################################################################
printStep(testname,'Step 2',\
          'STA1 connect to network 1,',\
          'STA1 dhcp and get 192.168.92.X ip address,',\
...
#check
res2 = CheckPing(pc1,sta1_ipv4,mode='linux',srcip=pc1_ipv4)
