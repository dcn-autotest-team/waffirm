...
#
#*******************************************************************************
# waffirm_4.3.5.py - test case 4.3.5 of waffirm
#
...
#
# Features:
# PA/WPA2-Personal混合模式下客户端认证接入测试
# 测试目的：测试客户端通过WPA/WPA2-Personal混合模式接入无线网络。
# 测试环境：同测试拓扑
# 测试描述：测试客户端通过WPA/WPA2-Personal混合模式接入无线网络。
#           Wpa version为wpa/wpa2混合模式。（STA1的MAC地址：STA1MAC）
#
#*******************************************************************************
# Change log:
#     - 
...
testname = 'TestCase 4.3.5'
printTimer(testname,'Start','Test wpa/wpa2-personal mode with wireless client')
...
for tmpCounter in xrange(0,10):
    ...
    if 0 == CheckLine(data,ap1mac_lower,'WPA-PSK-TKIP\+CCMP.+?WPA2-PSK-TKIP\+CCMP',Network_name1):
        break
...
#Step 3
#操作
#设置STA1无线网卡的属性为WPA2-PSK认证，正确输入密码，关联网络test1。
#预期
...
#Step 6
#操作
#设置STA1无线网卡的属性为WPA-PSK认证，关联网络test1，正确输入密码。
#预期
#成功关联，并获取192.168.91.X网段的IP地址。Show wireless client summery
#可以看到STA1（“MAC Address”显示“STA1MAC”），IP地址的网段正确。。
################################################################################
