...
#
#*******************************************************************************
# waffirm_4.30.py(waffirm_mt4762_WDS.py)
#
# Author:  (zhangjxp)
#
# Version 1.0.0
...
#
# Features:
# 4.30	WDS功能测试（测试内容与mt4762基本相同）
# 测试目的：Root AP和Satellite AP可以通过WDS链路建立连接
# 测试环境：同测试拓扑
# 测试描述：Root AP和Satellite AP可以通过WDS链路建立连接，
# 连接成功后Satellite AP被AC管理上。无线用户可以关联Root AP和
# Satellite AP上开启的VAP。（ap1为Root(mac地址为RootAP1_MAC)，
# ap2为Satellite(mac地址为SatelliteAP2_MAC)）
#
# Features:
...
checklist1.append(('wds-security-policy','wpa-personal'))
checklist1.append(('remote-mac',ap2MAC))
checklist1.append(('wds-wpa-personal-key','12345678'))
...
SetCmd(ap2,'set wds wds0 wds-security-policy wpa-personal')
SetCmd(ap2,'set wds wds0 remote-mac',ap1MAC)
SetCmd(ap2,'set wds wds0 wds-wpa-personal-key 12345678')
