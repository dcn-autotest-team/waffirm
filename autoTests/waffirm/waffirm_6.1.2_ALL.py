...
#
#*******************************************************************************
# waffirm_6.1.2.py - test case 6.1.2 of waffirm_new
#
# Author:  ()
#
# Version 1.0.0
...
#
# Features:
# 6.1.2	lan port vlan功能测试（三层转发）
# 测试目的：测试lan port vlan功能
# 测试环境：同测试拓扑
# 测试描述：测试pc连接lan口后三层数据能够被正常转发
#
#*******************************************************************************
# Change log:
...
#Functional Code
if hwtype1 == '33':  
	testname = 'TestCase 6.1.2'
	avoiderror(testname)
	printTimer(testname,'Start','test L3 switch in lan port mode')
	...
	# 在AC1 ap profile 1 中配置lan port 1 vlan 4091
	# 配置下发到AP1
	#预期
	#AP1能够被AC1管理，配置下发成功
	################################################################################
	...
	SetCmd(switch1,'lan port 1 vlan 4091')
	...
	SetCmd(switch3,'switchport trunk allowed vlan 20,4091,4092')
	EnterInterfaceMode(switch3,s3p1)
	SetCmd(switch3,'switchport mode trunk')
	SetCmd(switch3,'switchport trunk native vlan',Vlan40)
	SetCmd(switch3,'switchport trunk allowed vlan 40,4091,4092')
	...
	SetCmd(switch1,'switchport trunk allowed vlan 40,4091,4092')
	EnterWirelessMode(switch1)
	SetCmd(switch1,'no l2tunnel vlan-list')
	#配置下发到AP1
	res1 = WirelessApplyProfileWithCheck(switch1,['1'],[ap1mac])
	EnterEnableMode(switch1)
	SetCmd(ap1,'set lan-port status down')
	IdleAfter(15)
	SetCmd(ap1,'set lan-port status up')
	IdleAfter(15)
	...
	#AP3连接到ap1的lan1口
	#AP4连接到ap2的lan1口
	#注意：执行此用例请务必将标识PC2的网线连接到LAN1口，标识PC3的网线连接到LAN2口，不可连反！！！！！！
	#预期
	#关联成功，AP3能够获取192.168.91.X网段的IP地址
	#关联成功，AP4能够获取192.168.92.X网段的IP地址
	################################################################################
	printStep(testname,'Step 2',
              'AP3 connect to LAN1',
              'AP4 connect to LAN2',
              'Check if AP3 can obtain 192.168.91.x address via dhcp',
              'Check if AP4 can obtain 192.168.92.x address via dhcp')
	...
	#check
	SetCmd(pc1,'\n')
	SetCmd(pc1,'route add -net '+Dhcp_pool1+'/25 gw '+If_vlan4092_s1_ipv4)
	SetCmd(pc1,'route add -net '+Dhcp_pool2+'/25 gw '+If_vlan4091_s1_ipv4)
	res1 = CheckPing(pc1,pc2_ipv4,mode='linux',srcip=pc3_ipv4)
	#result
	...
	# 删除dhcp binding,arp,mac-address-table信息
	EnterEnableMode(switch1)
	...
	#S3 上删除 vlan 4092
	EnterConfigMode(switch3)
	SetCmd(switch3,'no vlan 4091;4092')
