...
# AC1(config)#wireless 
# AC1(config-wireless)# channel-plan bgn history-depth 0
# AC1(config-wireless)#channel-plan bgn refer-only-managed-ap
# AC1(config-wireless)#channel-plan bgn interval 4
# AC1(config-wireless)#channel-plan bgn mode interval
#预期
#配置成功
################################################################################
...
SetCmd(switch1,'channel-plan bgn history-depth 0')
SetCmd(switch1,'channel-plan bgn refer-only-managed-ap')
...
#operate
# IdleAfter(int(Bgn_interval+1)*60)
IdleAfter(60)
for i in range(30):
	data1 = SetCmd(switch1,'show wireless ap radio status',promotePatten='#')
...
SetCmd(switch1,'no channel-plan bgn refer-only-managed-ap')
SetCmd(switch1,'no channel-plan bgn history-depth')
#还原channel
EnterWirelessMode(switch1)
SetCmd(switch1,'ap database',ap1mac)
