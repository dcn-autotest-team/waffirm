...
#
# *******************************************************************************
# waffirm_4.11.2.py - test case 4.11.2 of waffirm_new
#
# Author:  ()
#
# Version 1.0.0
...
#
# Features:
# 4.11.2     VAP Profile组节点测试
# 测试目的：检查VAP Profile组私有mib节点。
# 测试环境：同测试拓扑
...
testname = 'TestCase 4.11.2'
avoiderror(testname)
printTimer(testname, 'Start', 'VAP PRofile group node test')
...
# 操作
# wsVAPId
# 读取vap id。Object ID =  1.3.6.1.4.1.6339.103.1.3.6.1.1。范围：0~15。
################################################################################
printStep(testname, 'Step 4',
          'wsVAPId')

data1 = SetCmd(pc1, 'snmpwalk -v 2c -c public ' + StaticIpv4_ac1 + ' ' + oid + '.103.1.3.6.1.1')

res1 = CheckLine(data1, 'INTEGER: 0')
res2 = CheckLine(data1, 'INTEGER: 15')
res3 = CheckLine(data1, 'INTEGER: 16')
...
# 操作
# wsVAPMode
# 读取/设置vap模式。Object ID =  1.3.6.1.4.1.6339.103.1.3.6.1.2。
# Value：1enable,2disable。缺省为1。
################################################################################
printStep(testname, 'Step 5',
          'wsVAPMode')

data1 = SetCmd(pc1, 'snmpwalk -v 2c -c public ' + StaticIpv4_ac1 + ' ' + oid + '.103.1.3.6.1.2')

res1 = CheckLine(data1, '1.1.0 = INTEGER: 1')
res2 = CheckLine(data1, '1.1.1 = INTEGER: 1')
...
# 操作
# wsVAPNetworkId
# 读取/设置vap的网络id。Object ID =  1.3.6.1.4.1.6339.103.1.3.6.1.3。范围：1~32。
################################################################################
printStep(testname, 'Step 6',
          'wsVAPNetworkId')

data1 = SetCmd(pc1, 'snmpwalk -v 2c -c public ' + StaticIpv4_ac1 + ' ' + oid + '.103.1.3.6.1.3')

res1 = CheckLine(data1, 'INTEGER: 1')
res2 = CheckLine(data1, 'INTEGER: 16')
res3 = CheckLine(data1, 'INTEGER: 17')
printCheckStep(testname, 'Step 6', res1, res2, not res3)
