...
#
#*******************************************************************************
# waffirm_4.8.3.py - test case 4.8.3 of waffirm
#
# Author: 
#
# Version 1.0.0
...
# Features:
# 4.8.3	计费功能测试-
# 测试目的：测试客户端通过Portal认证以后按照时长、流量计费正确
# 测试环境：同测试拓扑
# 测试描述：测试客户端通过Portal认证以后按照时长、流量计费正确
#
#*******************************************************************************
# Change log:
#     - 
...
testname = 'TestCase 4.8.3'
avoiderror(testname)
printTimer(testname,'Start','Test aaa-accounting')
...
if GetWhetherkeepon(keeponflag):
    ...
    if None != web:
        res1 = web_open(web,'http://2.2.2.2')
        if res1['status']:
    ...
    #Step 5
    #操作
    #在Portal服务器上过滤AC发出的计费更新报文，过滤2min
    #
    #预期
    #查看是否有目的端口号为1813的UDP报文，有则测试通过（该报文即是AC的计费更新报文）
    ################################################################################

    printStep(testname,'Step 5',
              'check whether AC send accounting update packets')
    res1=1
    #operate
    StartDebug(switch1)
    EnterEnableMode(switch1)
    SetCmd(switch1,'debug aaa detail event')
    IdleAfter(250)
    SetCmd(switch1,'no debug aaa detail event')
    data=StopDebug(switch1)
    res1=CheckLine(data,'ACCOUNTING_REQUEST','Interim-Update',ML=True)
    #result
