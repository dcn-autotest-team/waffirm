...
#
# Features:
# 3.2.1 10台ap（包含I3R2/I2R2/WL2/I2/79五类机型），同时被ac管理，并通过ac下发配置文件
# (包含3个radio、15个vap，覆盖射频管理、用户隔离、广播抑制、ap逃生、portal功能等配置)，
# 持续打入100pps的arp和dhcp广播报文，操作ap反复上下电1000次
#*******************************************************************************
# Change log:
...
def run(total_cycle_time):
    bssid_list = [ap1mac_type1]
    for ap in aplist:
        bssid_list.append(ap['mac_type1'])
    print('bssid_list=',bssid_list)
    threadslist = []
    t1 = CallThread(maintest,total_cycle_time)
    threadslist.append(t1)
    # t2 = CallThread(sta_connect_ssid_circularly,sta2,Netcard_sta2,Network_name1,bssidlist=bssid_list,printflag=False)
