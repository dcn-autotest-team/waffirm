...
#
# Features:
# 3.3.2 ap通过telnet登录，执行firmware-updata tftp进行ap直接升级，第一次执行telnet自动断开，
# 第二次执行telnet通过shutdown链路断开断开，依次反复执行1000次，每次都能升级成功，
...
testname = 'Performance_test 3.3.3'
printTimer(testname,'Start','AC telenet AP and firmware-upgrade ap\'s version 1000 times and ap behave correctly')
...
def maintest(total_cycle_time,**args):
    '''    
    函数：通过AC telnet AP升级AP版本，升级成功后，客户端关联AP并下载文件，循环执行上述过程
    ...
    for i in range(int(total_cycle_time/4)):
        for j in range(2):
            # 每次升级时对telnet连接有两种处理方式，一种是等待telnet连接自动断开，另一种是手动shutdown端口使telnet连接断开
            telnet_stopflag = 'auto' if j==0 else 'manual'
            for k in range(2):
                ...
                ################################################################################
                # step2:AC telenet登陆AP，升级AP版本，并检查是否升级成功
                ################################################################################
                printStep(testname,'Step 2','ac telnet ap,and upgrade ap version,then check ap version on ac')
                res2 = 1
                totaltime += 1
                data = SetCmd(switch1,'telnet '+Ap1_ipv4,promotePatten='login|Unable to connect',promoteTimeout=10)
                if CheckLine(data,'login') != 0:
                    print('AC can not telnet AP,please check!!!!!!!!!!!!')
                else:
                    res2a = TelnetLogin(switch1,'admin','admin')
                    if res2a == 0:
                        # 判断是等待telnet自动断开还是手动shutdown端口使telnet断开
                        StartDebug(switch1)
                        data = SetCmd(switch1,'firmware-upgrade tftp://'+updateserver+'/'+apversionfile,timeout=10)
                        if telnet_stopflag == 'auto':
                            print('telnet connection closed type is auto')
                            # data = SetCmd(switch1,'firmware-upgrade tftp://'+updateserver+'/'+apversionfile,promotePatten='Connection closed|(Firmware upgrade failed)',promoteTimeout=10)
                        else:
                            print('telnet connection closed type is manual')
                            # data = SetCmd(switch1,'firmware-upgrade tftp://'+updateserver+'/'+apversionfile,timeout=10)
                        ...
                        # elif CheckLine(data,'Connection closed') == 0 or telnet_stopflag == 'manual':
                        else:
                            SetCmd(ap1,'\n',promotePatten='(Starting kernel)|(Firmware upgrade failed)',promoteTimeout=360)
                            if telnet_stopflag == 'manual':
                                EnterConfigMode(switch3)
                                SetCmd(switch3,'interface',s3p1)
