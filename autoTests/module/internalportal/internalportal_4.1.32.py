...
#
# 4.1.32	开启mac-ip-based后的portal认证
...
# 2、	关闭mac-ip-based后，STA1连接网络进行portal认证，客户端的ip地址发生变化，AC上的CP表更新，客户端仍然处于认证状态
# 测试环境：见测试环境拓扑图1
#*******************************************************************************
# Change log:
...
testname = 'TestCase internalportal_4.1.32'
avoiderror(testname)
...
#Step 8
#操作
#STA1释放静态地址，连接到AP1的Network_name1
#预期
# 关联成功。
# 通过命令show wireless  client  summary可以查看客户端获取到192.168.X.X(Dhcp_pool1)网段的地址
################################################################################
...
#operate
#STA1关联 network1
SetCmd(sta1,'ifconfig',Netcard_sta1,'0.0.0.0')
res1 = WpaConnectWirelessNetwork(sta1,Netcard_sta1,Network_name1,checkDhcpAddress=Netcard_ipaddress_check,bssid=ap1mac_type1)
if res1 == 0:
    res2 = CheckWirelessClientOnline(switch1,sta1mac,'online')
#获取STA1的地址
