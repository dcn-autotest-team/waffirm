...
#
# Features:
# 4.1.17	客户端可以连续打开多个重定向页面
# 测试目的：AC1配置内置portal认证，客户端连接SSID后，可以连续打开多个重定向页面
# 测试描述：
# 1、	AC1上面配置内置portal认证，客户端连接SSID后，发起http请求后可以得到重定向页面。
# 2、	STA1不认证，重新打开web页面输入任意ip地址进行重定向
# 3、	STA1不认证，重新打开web页面输入任意ip地址进行重定向
...
testname = 'TestCase internalportal_4.1.17'
avoiderror(testname)
...
if GetWhetherkeepon(keeponflag):
    ...
    for i in range(3):
        ...
        res1 = inportal_redirect_success(web2,web_ip)
        if res1 == 0:
            break
        web_close(web2)
    #result
    ...
    for i in range(3):
        ...
        res1 = inportal_redirect_success(web3,web_ip)
        if res1 == 0:
            break
        web_close(web3)
    #result
    ...
    inportal_login_withcheck(web,portal_username,portal_password)
