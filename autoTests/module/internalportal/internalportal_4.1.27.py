...
#
# Features:
# 4.1.27	集中转发下的内置portal认证
# 测试目的：集中转发下，客户端连接网络能够正常进行内置portal认证
# 测试描述：
# 1、	修改测试环境为集中转发
# 2、	开启内置portal认证
...
testname = 'TestCase internalportal_4.1.27'
avoiderror(testname)
printTimer(testname,'Start','Test centralized forwarding')
###############################################################################
#Step 1
#操作
# AC1添加Vlan4091到集中隧道，AC1添加Vlan4091：
# AC1(config)#vlan Vlan4091
# AC1(config)#wireless
# AC1 (config-wireless)# l2tunnel vlan-list Vlan4091
# S1p1 ,s2p1,s3p1,s3p2改为trunk口，允许Vlan4091通过
# S3p3 access vlan 为vlan20，S3p4 access vlan为vlan3
#预期
# 设置成功
