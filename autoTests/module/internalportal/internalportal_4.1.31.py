...
#
# Features:
# 4.1.31	1+1场景下的portal认证
# 测试目的：开启1+1热备后，AP在主备AC之间切换过程中，客户端连接网络可以正常进行portal认证
# 测试描述：
# 1、	开启内置portal认证和1+1热备
# 2、	STA1连接网络进行portal认证，客户端STA2连接到网络不进行portal认证
# 3、	AP从主AC切换到备AC后，客户端STA2进行portal认证能够成功，同时STA1和STA2的表项都正常
# 4、	AP从备AC切换到主AC后，STA1和STA2的表项仍然正常
# 测试环境：见测试环境拓扑图1
#*******************************************************************************
# Change log:
...
testname = 'TestCase internalportal_4.1.31'
avoiderror(testname)
...
CheckSutCmd(switch1,'show wireless', \
                check=[('WS IP Address',S1_vlan30_ipv4)], \
...
CheckSutCmd(switch2,'show wireless', \
                check=[('WS IP Address',S2_vlan30_ipv4)], \
                retry=20,waitflag=False,IC=True) 
...
if GetWhetherkeepon(keeponflag):
    ...
    # 可以ping通
    ################################################################################
    printStep(testname,'Step 7','sta1 ping pc1 successully')
    res1=1
    # operate
    ...
    if sta2_login_flag == 0:
        ...
        ################################################################################
        printStep(testname,'Step 8','sta2 open 1.1.1.1 and can redirect to portal auth page')
        res1=1
        #operate
    ...
    # Master显示为30.1.1.10 (Active)
    ################################################################################
    printStep(testname,'Step 10','No shutdown s1p1 and shutdown s2p1',\
    ...
    # STA1和STA2都可以ping通PC1
    ################################################################################
    printStep(testname,'Step 11','sta1 and sta2 are still in portal client list',\
