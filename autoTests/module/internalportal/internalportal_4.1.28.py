...
# Features:
# 4.1.28	客户端下线功能
# 测试目的：内置portal认证场景下，客户端认证成功后，可以正常下线
# 测试描述：
# 1、	STA1接网络后输入正确的用户名和密码认证通过
# 2、	AC1上可以强制STA1下线
# 3、	STA1下线后再次发起认证请求可以得到重定向页面并且认证成功
...
testname = 'TestCase internalportal_4.1.28'
avoiderror(testname)
...
if GetWhetherkeepon(keeponflag):
    ...
    #预期
    # 绑定成功
    # 实例1下通过命令show running-config current-mode可以查看到interface ws-network 1
    # AC1上通过命令show captive-portal  aip status可以查看到AIP表项，其中ip=sta1_ipv4  mac=sta1_mac
    # STA1无法ping通pc1_ipv4
    ################################################################################
    ...
    res1 = CheckLine(data,'interface ws-network 1')
    IdleAfter(10)
