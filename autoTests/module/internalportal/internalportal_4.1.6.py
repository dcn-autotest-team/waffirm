...
#
# Features:
# 4.1.6	黑白名单url-filter-permit和url-filter deny
# 测试目的：验证内置portal认证场景下url黑白名单功能。
# 测试描述：
# 1、	开启内置portal认证,配置黑白名单，并且黑白名单绑定到实例下。
# 2、	STA1不通过认证可以访问白名单
# 3、	STA1通过认证不能访问黑名单
# 测试环境：见测试环境拓扑图1
#*******************************************************************************
...
testname = 'TestCase internalportal_4.1.6'
avoiderror(testname)
...
# S3(config)#ip dhcp pool pool4091
# S3(dhcp-pool4091-config)# dns-server DNS_server
# S3(dhcp-pool4091-config)#exit
# AC1(config)#url-filter permit 1 web_domain1                                                                                      
# AC1(config)#url-filter permit 2 web_domain2                                                                                      
# AC1(config)#url-filter deny 1 web_domain3                                                                                        
# AC1(config)#url-filter deny 2 web_domain4                                                                                        
...
# RuleID为1，host为web_domain1,action为permit
# RuleID为2，host为web_domain2,action为permit
# RuleID为1，host为web_domain3,action为deny
# RuleID为2，host为web_domain4,action为deny
################################################################################
...
SetCmd(switch1,'dns-server', DNS_server)
EnterConfigMode(switch1)
...
if GetWhetherkeepon(keeponflag):
    ...
    #Step 3
    #操作
    # 客户端STA1未认证通过，使用浏览器访问web_domain1
    #预期
    # web_domain1可以打开
    ################################################################################
    ...
    #Step 5
    #操作
    # 客户端STA1未认证通过，使用浏览器访问web_domain3
    #预期
    # web_domain3无法打开
    ################################################################################
    ...
    #Step 6
    #操作
    # 客户端STA1未认证通过，使用浏览器访问web_domain4
    #预期
    # web_domain4无法打开
    ################################################################################
    ...
    res1 = inportal_redirect_success(web1,web_ip)
    if res1 == 0:
        res2 = inportal_login_withcheck(web1,portal_username,portal_password)
