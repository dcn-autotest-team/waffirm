...
#
# Features:
# 4.1.8	http portal功能验证
# 测试目的：Portal用户在认证前，AP对目的端口为80的HTTP报文做重定向，当配置了额外的HTTP portal后，AP对目的端口为80和配置端口的HTTP做重定向 
# 测试描述：
# 1、	开启内置portal认证，STA1连接无线网络
# 2、	配置http port为80
# 3、	配置http port为1，STA1进行重定向
# 4、	配置http port为81，STA1进行重定向
# 5、	配置http port为65535，STA1进行重定向
# 6、	STA1使用一个没有添加过的端口进行重定向
#*******************************************************************************
# Change log:
...
testname = 'TestCase internalportal_4.1.8'
avoiderror(testname)
...
if GetWhetherkeepon(keeponflag): 
    ...
    res2 = inportal_redirect_success(web,web_ip)
    #result
    ...
    # web = web_init(sta1_host)
    # res2 = inportal_redirect_success(web,'http://1.1.1.1:1')
    # # 关闭网页
    # web_close(web)
    # # sta1打开1.1.1.1，并检查是否重定向到portal认证页面
    # web = web_init(sta1_host)
    ...
    web = web_init(sta1_host)
    res2 = inportal_redirect_success(web,'http://1.1.1.1:81')
    # 关闭网页
    web_close(web)
    # sta1打开1.1.1.1，并检查是否重定向到portal认证页面
    web = web_init(sta1_host)
    ...
    web = web_init(sta1_host)
    res2 = inportal_redirect_success(web,'http://1.1.1.1:65535')
    # 关闭网页
    web_close(web)
    # sta1打开1.1.1.1，并检查是否重定向到portal认证页面
    web = web_init(sta1_host)
    ...
    web = web_init(sta1_host)
    res1 = inportal_redirect_success(web,'http://1.1.1.1:100')
