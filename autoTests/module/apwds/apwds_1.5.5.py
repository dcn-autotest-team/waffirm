...
#
# Features:
# 1.5.5	rootap国家码cn，satelliteap国家码us，2.4G/5G能建立wds
#
#*******************************************************************************
# Change log:
...
testname = 'TestCase apwds_1.5.5'
avoiderror(testname)
printTimer(testname,'Start','Rootap country code is cn, satelliteap country code is us,\n \
                            WDS link can be established in open mode and radio 2.4G/5G')
###############################################################################
#Step 1
#操作
# AC1上配置国家码为cn
...
# 配置成功
################################################################################
printStep(testname,'Step 1','Config country-code cn on AC1',\
...
res1 = CheckSutCmd(ap2,'get system detail',\
                    check=[('country','US')],retry=5,waitflag=False,IC=True)
#result
...
#检查WDS连接能够建立
#预期
# AP1，AP2上查看wds-link状态为unlinked
################################################################################

printStep(testname,'Step 3',\
                    'Check WDS is unlinked')
