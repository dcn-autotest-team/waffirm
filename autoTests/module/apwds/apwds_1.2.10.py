...
testname = 'TestCase apwds_1.2.10'
avoiderror(testname)
printTimer(testname,'Start','Test apply ap profile to satelliteap')
...
SetCmd(switch1, 'wireless ap profile apply 2', promotePatten='Y/N',promoteTimeout=10)
SetCmd(switch1,'y')
res1 = CheckSutCmd(ap2,'get radio '+otherwlan,\
                    check=[('\nstatus','down')],retry=50,interval=1,waitflag=False)
res2 = CheckSutCmd(ap2,'get radio '+wlan,\
...
#Step 5
#操作
#AP2上检查vap 0的ESSID 为xxxxx
#预期
#ESSID显示为xxxxx
################################################################################
printStep(testname,'Step 5',\
                    'Check AP2 vap0 Essid is xxxxx')
res1=1
# operate
data = SetCmd(ap2,'iwconfig ath0')
