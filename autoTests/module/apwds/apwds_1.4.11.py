...
#
# Features:
# 1.4.11	wpa-personal方式下建立wds,修改satelliteap的remote-mac为空，wds正常
#
#*******************************************************************************
# Change log:
...
testname = 'TestCase apwds_1.4.11'
avoiderror(testname)
printTimer(testname,'Start','Change remote-mac of satelliteap then psk mode wds should be still linked')
###############################################################################
#Step 1
#操作
# AP2上进行psk模式的WDS配置
...
#操作
#AP2上修改remote-mac为空
# set wds wds0 remote-mac
#预期
# 等待30秒，WDS正常
################################################################################
printStep(testname,'Step 3',\
                    'Change remote-mac of AP2',\
                    'WDS should be still linked')
res1=res2=res3=res4=1
#operate
Ap_set_wds(ap2,Ap2cmdtype,wds0num,'remote-mac')
IdleAfter(30)
res1 = CheckSutCmd(ap1,'get wds '+wds0num,\
                    check=[('wds-link','linked')],retry=1,waitflag=False)
