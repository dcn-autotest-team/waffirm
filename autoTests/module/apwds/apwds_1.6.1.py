...
#
# Features:
# 1.6.1	胖AP模式，open方式下2.4G/5G建立wds连接
#
#*******************************************************************************
# Change log:
...
testname = 'TestCase apwds_1.6.1'
avoiderror(testname)
printTimer(testname,'Start','Config ap as fat ap,establish WDS link in open mode and radio 2.4G/5G')
###############################################################################
#Step 1
#操作
# 将s3p3 shutdown
...
# 配置成功
################################################################################
printStep(testname,'Step 2','AP1 and AP2 config wds0 as open mode')
res1=1
#operate
# AP1上配置wds0,mode为rootap
Ap_openwds_config(ap1,Ap1cmdtype,
                    wds0num,
                    ssid=wds_ssid,
