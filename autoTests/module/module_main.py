...
# 导入参数    
exec(compile(open('module_config_topu.py', "rb").read(), 'module_config_topu.py', 'exec'))
exec(compile(open('module_config_vars.py', "rb").read(), 'module_config_vars.py', 'exec'))
exec(compile(open('module_config_run.py', "rb").read(), 'module_config_run.py', 'exec'))

# 获取用户自定义的字符串，添加在log的文件名中
if 'prefix_log' in vars():
    if prefix_log == '':
        pass
    else:
...
for module in module_list:
    ...
    # 导入用例
    exec(compile(open(module + '\\' + module + '_testcase.py', "rb").read(), module + '\\' + module + '_testcase.py', 'exec')) 
    # 判断需要执行的测试类别和测试用例
    testcategory_list = get_testcase_byuser()
    ...
    for item in range(len(testcategory_list)):
        ...
        if '5G' in testcategory_list[item]:
            ...
            wlan = 'wlan1'
        # 导入参数并执行初始化
        exec(compile(open(module + '\\' + module + '_vars.py', "rb").read(), module + '\\' + module + '_vars.py', 'exec'))
        # 开始执行用例
        if len(finaltestlist) > 0:
            ...
            for j in range(runtimes):
                ...
                for case in finaltestlist:
                    try:
                        ...
                        exec(compile(open(module + '\\' +case[0], "rb").read(), module + '\\' +case[0], 'exec'))
                    except Exception as e:
                        error_fix(case[0])
            ...
            if len(faillister) > 0:
                ...
                for fileTemp in faillister:
                    try:
                        exec(compile(open(module + '\\' +fileTemp, "rb").read(), module + '\\' +fileTemp, 'exec'))
                    except Exception as e:
                        error_fix(fileTemp)
                printGlobal('TestCase_'+testName,'RerunEnd')
            if set_default != 1:
                exec(compile(open(module + '\\' + module + '_uninitial.py', "rb").read(), module + '\\' + module + '_uninitial.py', 'exec'))