...
#
#*******************************************************************************
# clustermanagement_3.1.1.py - test case 3.1.1 of clustermanagement
#
# Author:  humj
#
# Version 1.0.0
...
#
# Features:
#3.1.1 AC开启无线IP地址自动选择功能测试
# 测试目的：测试AC开启IP地址自动选择功能后，AC是否能自动选择IP地址提供无线服务。
# 测试环境：同测试拓扑
...
testname = 'TestCase clustermanagement_3.1.1'

avoiderror(testname)
printTimer(testname,'Start','Test AC automatically choose wireless IP Address')
...
#
#预期
#AC1的无线功能不能开启成功,show wireless看到'WLAN Switch Disable Reason'项已经显示为'IP address not configured'
################################################################################
...
#
#预期
#AC1会自动选择索引号最小的三层接口IP为无线模块的IP地址
#AC1上show wireless看到'WS IP Address'项已经显示为'If_vlan8_ipv4_s'
################################################################################
...
#check
res1 = CheckSutCmd(switch1,'show wireless',
                   check=[('WS IP Address',If_vlan8_ipv4_s)],
                   retry=3,waitflag=False,IC=True)
...
#
#操作
#把选定为无线模块IP地址的三层接口down掉
#AC会重新自动选择剩余up的接口索引号最小的接口对应的IP地址为无线模块的IP地址
#
#预期
...
#
#预期
#AC1上show wireless看到'WS IP Address'项已经显示为'If_loopback3_ipv4_s'
################################################################################
...
SetCmd(switch1,'ip address',If_loopback3_ipv4)
IdleAfter(10)
...
#check
res1 = CheckSutCmd(switch1,'show wireless',
                   check=[('WS IP Address',If_loopback3_ipv4_s)],
                   retry=5,waitflag=False,IC=True)
...
#
#操作
# 删除以上所有三层接口和环回接口上的IP地址
#
#预期
#AC1上show wireless看到'WS IP Address'项已经显示为'-----','WLAN Switch Disable Reason'显示为'IP address not configured'
################################################################################

printStep(testname,'Step 9',
          'Delete all the ip address on interface',
          'Check the result')
...
#check
res1=CheckSutCmd(switch1,'show wireless',
                 check=[('WS IP Address','-----'),('WLAN Switch Disable Reason','IP address not configured')],
                 retry=5,interval=5,IC=True)
