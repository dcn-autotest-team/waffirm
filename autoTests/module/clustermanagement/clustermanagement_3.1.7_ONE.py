...
#
#*******************************************************************************
# clustermanagement_3.1.7.py - test case 3.1.7 of clustermanagement
#
# Author:  humj
#
# Version 1.0.0
...
#
# Features:
# 3.1.7 自动发现功能AC上discovery ip list列表测试
# 测试目的：测试AC上discovery ip list列表 
# 测试环境：同测试拓扑
...
testname = 'TestCase clustermanagement_3.1.7'

avoiderror(testname)
printTimer(testname,'Start','Test discovery ip list on AC')
...
#
#操作
# 在AC1 ip list列表中添加一个地址
#
#预期
# 用show wireless discovery ip-list查看检查：
# Total Number of Configured Entries值为1;IP Address 显示为1.1.1.1
################################################################################
...
data1 = SetCmd(switch1,'show wireless discovery ip-list')
#check
res1 = CheckLineList(data1,[('Total Number of Configured Entries','1'),('1.1.1.1')],IC=True)
...
# operate
EnterWirelessMode(switch1)
SetCmd(switch1,'no discovery ip-list 2.2.2.2')
...
# operate		  
EnterWirelessMode(switch1)
SetCmd(switch1,'discovery ip-list 2.2.2.2')

EnterWirelessMode(switch1)
SetCmd(switch1,'discovery ip-list 3.3.3.3')
