...
#
# Features:
# 4.1.7.2	portal逃生恢复用户不下线
# 测试目的：外置portal场景下，开启portal逃生，取消portal逃生恢复强制客户端下线功能，当AC1与portal服务器的通信中断后，客户端连接网络后可以直接上网，逃生前认证过的用户，portal逃生恢复后仍然可以上网
# 测试描述：
# 1.	配置portal逃生功能，取消portal逃生恢复后强制客户端下线功能
# 2.	STA1连接网络进行portal认证
# 3.	中断AC1跟portal服务器的通信
# 4.	STA2连接无线网络，可以直接上网
# 5.	portal逃生恢复后，STA1仍然可以上网
# 测试环境：见测试环境拓扑图1
#*******************************************************************************
# Change log:
...
testname = 'TestCase externalportal_4.1.7.2'
avoiderror(testname)
...
# AC1(config-cp)#no portal-server-detect client-deauth
#预期
# 配置成功。
...
SetCmd(switch1,'no portal-server-detect client-deauth')
# check
...
SetCmd(switch1,'portal-server-detect client-deauth')
