...
SetCmd(switch2,'no configuration 1')
SetCmd(switch2,'no external portal-server ipv4 server-name eportal')
SetCmd(switch2,'no free-resource 1')
EnterConfigMode(switch2)
SetCmd(switch2,'no radius source-ipv4')
SetCmd(switch2,'no radius-server key')
SetCmd(switch2,'no aaa group server radius wlan')
SetCmd(switch2,'no radius nas-ipv4')
SetCmd(switch2,'no aaa enable')
SetCmd(switch2,'no aaa-accounting enable')
SetCmd(switch2,'no radius-server authentication host ' + Radius_server)
SetCmd(switch2,'no radius-server accounting host ' + Radius_server)
#wireless mode
...
if testcentral == True:
    ...
    SetCmd(switch3,'no ip dhcp pool vlan4093')
    SetCmd(switch3,'no ip dhcp excluded-address ' + If_vlan4091_s1_ipv4)
    SetCmd(switch3,'no ip dhcp excluded-address ' + If_vlan4092_s1_ipv4)
    SetCmd(switch3,'no ip dhcp excluded-address ' + If_vlan4093_s1_ipv4)
    # vlan
    SetCmd(switch3,'no interface vlan',Vlan4091)
    SetCmd(switch3,'no interface vlan',Vlan4092)
    SetCmd(switch3,'no interface vlan',Vlan4093)
    SetCmd(switch3,'no','vlan',Vlan4091)
    SetCmd(switch3,'no','vlan',Vlan4092)
    SetCmd(switch3,'no','vlan',Vlan4093)
    #Interface
    EnterConfigMode(switch3)
