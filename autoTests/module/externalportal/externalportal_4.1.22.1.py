...
#
# Features:
# 4.1.22.1	CP用户最大上行发送带宽和下行接收带宽功能测试
# 测试目的：外置portal认证场景下，AC可以限制认证客户端STA网络上行、下行数据的最大速率
# 测试描述：
...
testname = 'TestCase externalportal_4.1.22.1'
avoiderror(testname)
...
if GetWhetherkeepon(keeponflag):
    ...
    if res1 == 0:
        ...
        res1=res2=res3=1
        # operate
        res1 = exportal_login_withcheck(web,bandtest_username,bandtest_passwork)
    ...
    #预期
    # 下载成功，速率显示正常
    # 平均下载速率为32KB/s（误差不超过5%）(0.27-0.34)
    ################################################################################
    ...
    print('rate1=',rate1)
    print('rate2=',rate2)
    if 0.25 <= rate1 <= 0.36 or 0.25 <= rate2 <= 0.36:
        res1 = 0
    #result
    ...
    #预期
    #下载成功，速率显示正常
    # 平均下载速率为64KB/s（误差不超过5%）(0.54-0.68)
    ################################################################################
    ...
    print('rate1=',rate1)
    print('rate2=',rate2)
    if 0.52 <= rate1 <= 0.71 or 0.52 <= rate2 <= 0.71:
        res1 = 0
    #result
    ...
    if res3 == 0:
        res4 = exportal_login_withcheck(web,bandtest_username,bandtest_passwork)
