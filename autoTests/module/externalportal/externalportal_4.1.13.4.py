...
#
# Features:
# 4.1.13.4	WPA-Enterprise模式下客户端外置portal认证(2.4GHz覆盖)
# 测试目的：验证SSID采用WPA-Enterprise加密后开启portal认证，客户端连接网络能够正常进行外置portal认证
# 测试描述：
# 1.	修改network1的加密方式为WPA-Enterprise
...
testname = 'TestCase externalportal_4.1.13.4'
avoiderror(testname)
printTimer(testname,'Start','Test portal with WPA-Enterprise mode')
###############################################################################
#Step 1(合并方案step1,2)
#操作
# AC1上设置network1的加密方式为混合模式：WPA-Enterprise，WPA version为WPA，认证服务器使用wlan：                                                                                                                   
# AC1(config)#wireless                                                                                                                      
...
res4 = CheckLine(data,'WPA Versions','WPA2')
res4 = 0 if res4!=0 else -1
res5 = CheckLine(data,'WPA Versions','WPA')
res6 = WirelessApplyProfileWithCheck(switch1,['1'],[ap1mac])
#result
printCheckStep(testname, 'Step 1 Step 2',res1,res2,res3,res4,res5,res6)
###############################################################################
#Step 2
#操作
# 设置STA1无线网卡的属性为WPA-Enterprise认证，关联网络Network_name1，使用在Radius服务器配置的用户名和密码。
#预期
