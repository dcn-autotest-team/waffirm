...
# Features:
# 4.1.5	mac portal认证
# 测试目的：验证开启mac portal认证后，被添加到mac portal know表项中的客户端可以直接通过认证
# 测试描述：
# 1、开启mac portal认证功能
# 2、STA1的mac添加到mac portal know表项，STA2的mac不添加到mac portal know列表
...
testname = 'TestCase externalportal_4.1.5'
avoiderror(testname)
...
if GetWhetherkeepon(keeponflag):
    ...
    # STA2ping PC1
    #预期
    # 3、通过show captive-portal  client  status命令查看CP client列表，显示出STA1的信息（“MAC Address”显示“STA1MAC”）,
    # 列表中没有STA2的信息
    # 2、STA1ping PC1可以ping通
    # 3、STA2无法ping通PC1
    ################################################################################
    ...
    #Step 4
    #操作
    # 客户端STA2打开web页面访问1.1.1.1（该地址为变量web_ip），使用正确的用户名和密码登录
