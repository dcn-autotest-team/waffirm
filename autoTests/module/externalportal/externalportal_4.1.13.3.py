...
#
# Features:
# 4.1.13.3	WPA/WPA2-Personal混合模式下客户端外置portal认证(2.4GHz覆盖)
# 测试目的：验证SSID采用WPA/WPA2-Personal加密后开启portal认证，客户端连接网络能够正常进行外置portal认证
# 测试描述：
# 1.	修改network1的加密方式为WPA/WPA2-Personal
...
testname = 'TestCase externalportal_4.1.13.3'
avoiderror(testname)
printTimer(testname,'Start','Test portal with wpa/wpa2-personal mode')
###############################################################################
#Step 1(合并方案step1,2)
#操作
# AC1上设置network1的加密方式为混合模式：WPA/WPA2-Personal，密码为abcd1234                                                                                                                    
# AC1(config)#wireless                                                                                                                      
...
res2 = CheckLine(data1,'WPA Versions')
res3 = WirelessApplyProfileWithCheck(switch1,['1'],[ap1mac])
# result
printCheckStep(testname, 'Step 1 Step 2',res1,res2,res3)
###############################################################################
#Step 2
#操作
#设置STA1无线网卡的属性为WPA2-PSK认证，正确输入密码，关联网络Network_name1
#预期
...
res1=res2=1
#operate
res1 = WpaConnectWirelessNetwork(sta1,Netcard_sta1,Network_name1,connectType='wpa2_psk',psk='abcd1234',checkDhcpAddress=Netcard_ipaddress_check,bssid=ap1mac_type1)
if res1==0:
