...
#
# Features:
# 4.1.22.2	CP用户最大输入/发送字节和最大输出/接收字节功能测试(2.4GHz覆盖)
# 测试目的：外置portal认证场景下，如果AC上面配置了CP用户最大输入/发送字节和最大输出/接收字节，
# 那么客户端通过认证后的上传、下载流量超过限制后，客户端自动下线。需要注意的是，最大字节数也
# 是受radius侧的流量策略（资费组配置——控制配置——用量配置——组内帐号流量控制）控制的，radius的
# 配置优先级高于AC，只不过目前radius没有打开这个控制开关，才使得AC的配置可以生效。
# 测试描述：
...
#*******************************************************************************
# Change log:
#     - created by zhangjxp 2017.12.26
#*******************************************************************************

# ！！！注意1：实际测试中，从AC检测到客户端流量超标到强制客户端下线生效需要一定的时间，
# 在这段时间内客户端可能已经完成了文件的下载，因此脚本中不对文件是否下载成功做检查
# ！！！注意2：sta1从pc1下载文件，AC上的流量统计显示在Bytes Transmitted，对应Max Output Octets
# pc1从sta下载文件，AC上的流量统计显示在Bytes Received，对应Max Input Octets
...
def check_download(client,serverip,filename):
    ...
    if temp:
        ...
        return 1
testname = 'TestCase externalportal_4.1.22.2'
avoiderror(testname)
printTimer(testname,'Start','Test max-input-octets and max-output-octets')
################################################################################
#Step 1
#操作
# AC1上配置CP用户最大输入/发送字节为15MB最大输出/接收字节为10MB
# AC1(config)#captive-portal                                                                                                          
# AC1(config-cp)#configuration 1                                                                                                      
# AC1(config-cp-instance)# max-input-octets 15728640                                                                       
# AC1(config-cp-instance)# max-output-octets 10485760
#预期
# 配置成功。
# AC1上通过命令show captive-portal  configuration  1 status  可以查看到
# Max Input Octets为15728640
# Max Output Octets为10485760
################################################################################
...
if GetWhetherkeepon(keeponflag):
    ...
    if download_flag == 0:
        ...
        #Step 5
        #操作
        # 客户端STA1从有线端PC上下载15M的文件，查看文件下载是否成功，STA1是否仍然处于认证状态  
        #预期
        # 文件下载失败，STA1处于非认证状态
    ...
    res1 = exportal_redirect_success(web,web_ip)
    if res1 == 0:
        res2 = exportal_login_withcheck(web,portal_username,portal_password)
    ...
    if download_flag == 0:
        ...
        #Step 7
        #操作
        # 有线端PC从客户端STA1上下载10M的文件，查看文件下载是否成功，STA1是否仍然出入认证状态 
        #预期
        # 文件下载失败，STA1处于非认证状态
        # AC上面通过命令show captive-portal  client  status查看不到任何表项
        ################################################################################
    ...
    # Max Total Octets为0
    ################################################################################
    printStep(testname,'Step 8','No max-input-octets and max-output-octets')
    res1=1
    #operate
    EnterConfigMode(switch1)
    SetCmd(switch1,'captive-portal')
