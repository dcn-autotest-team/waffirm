...
#
# Features:
# 4.1.17.3	客户端在开启portal认证的network(SSID相同)间切换网络
# 测试目的：验证客户端在开启了portal认证的网络间的切换功能
# 测试描述：
# 1.	新建1个network2（SSID与network1相同），绑定到portal模块的实例下面
...
testname = 'TestCase externalportal_4.1.17.3'
avoiderror(testname)
...
if res1 == 0:
    ...
    res2 = CheckWirelessClientOnline(switch1,sta1mac,'online')
    # 通命令show wireless  client  status可查看到管理的SSID为 Network_name1
