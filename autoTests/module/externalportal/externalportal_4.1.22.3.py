...
#
# Features:
# 4.1.22.3 CP用户最大发送和接收字节总量功能测试(2.4GHz覆盖)
# 测试目的：外置portal认证场景下，如果AC上面配置了CP用户最大发送和接收字节总量，
# 那么客户端通过认证后的上传、下载流量之和超过限制后，客户端自动下线。需要注意的是，
# 总字节数属性也是受radius控制的，radius的配置优先级高于AC，只不过目前radius上没把相应的开关打开。
# 测试描述：
...
#*******************************************************************************
# 注意：实际测试中，从AC检测到客户端流量超标到强制客户端下线生效需要一定的时间，在这段时间内客户端可能已经完成了文件的下载，
# 因此脚本中不对文件是否下载成功做检查
#Package
...
if GetWhetherkeepon(keeponflag):
    ...
    if download_flag == 0:
        ...
        #Step 5
        #操作
        # 客户端STA1从有线端PC上下载5M的文件，查看文件下载是否成功，STA1是否仍然处于认证状态
        #预期
        # 文件下载成功，STA1处于认证状态
        # 通过show captive-portal  client  status命令查看CP client列表，显示出STA1的信息（“MAC Address”显示“STA1MAC”）
        # STA1ping PC1可以ping通
        ...
        printStep(testname,'Step 5',\
                            'sta1 download file(5M) from pc1 successully',\
                            'sta1 is still in captive-portal  client list')
        res1=res2=1
        #operate
        check_download(sta1,pc1_ipv4+':90','upgrade1.tar')
        IdleAfter(10)
        res2 = CheckSutCmd(switch1,'show captive-portal client status',\
                                    check=[(sta1mac)],retry=5,waitflag=False)
        #result
        ...
        #Step 6
        #操作
        # 有线端PC从客户端STA1上下载5M的文件，查看文件下载是否成功，STA1是否仍然出入认证状态
        #预期
        # 文件下载失败，STA1处于非认证状态
        # AC上面通过命令show captive-portal  client  status查看不到任何表项
        ################################################################################
        printStep(testname,'Step 6',\
                            'pc1 download file(5M) from sta1 failed',\
