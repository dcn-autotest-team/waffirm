...
#
# Features:
# 2.2.4 开放式认证+WEP加密：WEP认证无法关联成功
# 测试描述：开放式认证+WEP加密：client使用WEP认证，使用正确的密钥无法关联成功
#*******************************************************************************
# Change log:
...
testname = 'TestCase 802.11I_2.2.4'
avoiderror(testname)
printTimer(testname,'Start','client can not connect to ap with wep open mode when using wep shared auth method')
...
res1 = WpaConnectWirelessNetwork(sta1,Netcard_sta1,Network_name1,connectType='custom',
                                ...
                                key_mgmt='NONE',wep_key0=web_key1_hex,auth_alg='SHARED',
                                wep_key_type='hex',wep_tx_keyidx='0',bssid=ap1mac_type1)
