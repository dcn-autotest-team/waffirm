...
#
# Features:
# 2.7.4 WPA-802.1x+TKIP：wpa-psk方式关联失败
# 测试描述：WPA-802.1x+TKIP：client使用wpa-psk方式，使用正确的用户名密码时，关联失败
#*******************************************************************************
# Change log:
...
testname = 'TestCase 802.11I_2.7.4'
avoiderror(testname)
printTimer(testname,'Start','client can not connect to ap with wpa-enterprise TKIP mode when using using wpa-psk auth method')
...
res1 = WpaConnectWirelessNetwork(sta1,Netcard_sta1,Network_name1,connectType='custom',
                                ...
                                psk=Dot1x_password,bssid=ap1mac_type1)
