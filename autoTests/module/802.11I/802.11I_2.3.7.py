...
#
# Features:
# 2.3.7  WEP加密的Share Key：密钥匹配
# 测试描述：设置wep的4个key分别为key1，key2，key3，key4，设置认证加密使用key2，
#           客户端设置4个key为key2，key5，key6，key7，不能关联上AP
#*******************************************************************************
# Change log:
...
#Global Definition
web_key5_hex,web_key6_hex,web_key7_hex='2222222222','3333333333','4444444444'
#Source files
...
testname = 'TestCase 802.11I_2.3.7'
avoiderror(testname)
printTimer(testname,'Start','Config four keys:key1,key2,key3,key4 in wep share key mode network1\n \
                    and wep tx-key 2 on AC1\n \
                    Sta use four keys:key2,key5,key6,key7 to connect to network1 and connect failed')
...
#AC1(config-network)#wep tx-key 2
################################################################################
...
res5 = CheckLine(data,'WEP Transfer Key Index','\s2\s')
...
#Step 2
#操作
#设设置STA1为WEP Share Key认证，设置4各密码分别为key2,key5,key6,key7，关联test1
#
#预期
#关联失败。
################################################################################
printStep(testname,'Step 2',\
          'STA1 config 4 keys:key2,key5,key6,key7 to connect to network 1,',\
          'connect failed.')
res1=1
# operate
res1 = WpaConnectWirelessNetwork(sta1,Netcard_sta1,Network_name1,connectType='custom',
                                ...
                                wep_key0=web_key2_hex,wep_key1=web_key5_hex,
                                wep_key2=web_key6_hex,wep_key3=web_key7_hex,wep_tx_keyidx='1',
                                bssid=ap1mac_type1)
