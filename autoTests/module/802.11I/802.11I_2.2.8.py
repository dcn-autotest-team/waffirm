...
#
# Features:
# 2.2.8 开放式认证+WEP加密：wpa2-enterprise认证无法关联成功
# 测试描述： 开放式认证+WEP加密：client使用wpa2-enterprise认证，使用正确的密钥无法关联成功
#*******************************************************************************
# Change log:
...
testname = 'TestCase 802.11I_2.2.8'
avoiderror(testname)
printTimer(testname,'Start','client can not connect to ap with wep open mode when using wpa2-enterprise auth method')
...
#Step 2
#操作
#设置STA1为wpa2-enterprise方式认证
#
#预期
#客户端关联失败
################################################################################
printStep(testname,'Step 2',\
          'STA1 connect to network 1 using wpa2-enterprise authentication method',\
          'connect failed')
res1=1
# operate
res1 = WpaConnectWirelessNetwork(sta1,Netcard_sta1,Network_name1,connectType='custom',
                                dhcpFlag=False,reconnectflag=0,key_mgmt='WPA-EAP',
                                proto='WPA2',pairwise='CCMP',group='CCMP',eap='PEAP',
                                identity=Dot1x_identity,password=Dot1x_password,bssid=ap1mac_type1)
