...
#
# Features:
# 2.2.5 开放式认证+WEP加密：wpa-psk认证无法关联成功
# 测试描述： 开放式认证+WEP加密：client使用wpa-psk认证，使用正确的密钥无法关联成功
#*******************************************************************************
# Change log:
...
testname = 'TestCase 802.11I_2.2.5'
avoiderror(testname)
printTimer(testname,'Start','client can not connect to ap with wep open mode when using wpa_psk auth method')
...
res1 = WpaConnectWirelessNetwork(sta1,Netcard_sta1,Network_name1,connectType='custom',
                                ...
                                psk=web_key1_hex,bssid=ap1mac_type1)
