...
#
# Features:
# 2.6.4 非漫游关联成功后的处理:关联成功后,client和SSID对应关系正确
# 测试描述：在关联AP的管理UWS上查看SSID-Client Mapping表，该client和SSID的对应关系正确
#*******************************************************************************
# Change log:
...
testname = 'TestCase 802.11I_2.6.4'
avoiderror(testname)
...
if GetWhetherkeepon(keeponflag):
    ...
    #Step 3
    #操作
    #AC上上查看SSID-Client Mapping表
    #预期
    #该client和SSID的对应关系正确
    ################################################################################
    printStep(testname,'Step 3',\
              'Check SSID-Client Mapping',
              'sta1\'s ssid is test1')
    res1=1
    # operate
    res1 = CheckSutCmd(switch1,'show wireless client status ssid '+Network_name1,
                        check=[(Network_name1,sta1mac)],retry=10,waitflag=False)
