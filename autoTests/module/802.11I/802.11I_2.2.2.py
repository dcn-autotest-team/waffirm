...
#
# Features:
# 2.2.2 开放式认证+WEP加密：错误密钥可以关联无法通信
# 测试描述：开放式认证+WEP加密：client使用开放式认证+WEP加密，使用错误的密钥可以关联成功，但无法进行通信
#*******************************************************************************
# Change log:
...
testname = 'TestCase 802.11I_2.2.2'
avoiderror(testname)
printTimer(testname,'Start','client can not connect to ap with wep mode when using wrong key')
...
res1 = WpaConnectWirelessNetwork(sta1,Netcard_sta1,Network_name1,connectType='custom',
                                key_mgmt='NONE',wep_key0='1111111110',wep_key_type='hex',wep_tx_keyidx='0',
                                checkDhcpAddress=Netcard_ipaddress_check,bssid=ap1mac_type1,reconnectflag=0)
