...
# 测试描述：WPA2-802.1x-CCMP：client使用wpa2-enterprise方式，使用错误的用户名，正确的密码时，关联失败
#*******************************************************************************
# Change log:
...
testname = 'TestCase 802.11I_2.11.2'
avoiderror(testname)
printTimer(testname,'Start','client can not connect to ap with wpa2-enterprise CCMP mode when using wrong username')
...
#Step 2
#操作
#设置STA1无线网卡的属性为WPA2-Enterprise CCMP认证，输入错误的用户名和正确的密码，关联网络test1。
#预期
#关联失败
################################################################################
printStep(testname,'Step 2',\
          'STA1 connect to network 1 using wrong username',\
          'connect failed')

res1 = WpaConnectWirelessNetwork(sta1,Netcard_sta1,Network_name1,connectType='custom',
                                dhcpFlag=False,reconnectflag=0,
                                key_mgmt='WPA-EAP',proto='WPA2',pairwise='CCMP',group='CCMP',
                                eap='PEAP',identity='wawa',password=Dot1x_password,
                                bssid=ap1mac_type1)
