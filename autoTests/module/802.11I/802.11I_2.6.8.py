...
#
# Features:
# 2.6.8 非漫游关联成功后的处理:关联成功后,client加入Detected Clients表
# 测试描述：在关联AP的管理UWS上查看Detected Clients表，该client被加入到该表中，并且被标示为认证成功
#*******************************************************************************
# Change log:
...
testname = 'TestCase 802.11I_2.6.8'
avoiderror(testname)
printTimer(testname,'Start','Test Detected Clients table')
...
if GetWhetherkeepon(keeponflag):
    ...
    #预期
    #表项中存在sta1mac,状态为Authenticated
    ################################################################################
    printStep(testname,'Step 3',\
              'show wireless client detected-client status on AC1',
              'sta1mac is in list and it\'s status is Authenticated')
    res1=1
    # operate
    res1 = CheckSutCmd(switch1,'show wireless client detected-client status',
                        check=[(sta1mac,'Authenticated')],retry=10,waitflag=False)
