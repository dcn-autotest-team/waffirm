...
# 测试描述：设置wep的4个key分别为key1，key2，key3，key4，设置认证加密使用key3，
#           客户端设置4个key为key5，key6，key3，key7，能够关联上AP，可以ping通AP
#*******************************************************************************
# Change log:
...
#Global Definition
web_key5_ascii,web_key6_ascii,web_key7_ascii='11111','22222','44444'
#Source files
...
testname = 'TestCase 802.11I_2.3.10'
avoiderror(testname)
printTimer(testname,'Start','Config four keys:key1,key2,key3,key4 in wep share key mode network1 \n \
                    and wep tx-key 3 on AC1\n \
                    Sta use four keys:key5,key6,key3,key7 to connect to network1 and connect successfully')
...
#AC1(config-network)#wep tx-key 3
################################################################################
...
res5 = CheckLine(data,'WEP Transfer Key Index','\s3\s')
...
#Step 2
#操作
#设设置STA1为WEP Share Key认证，设置4各密码分别为key5,key6,key3,key7，关联test1
#
#预期
...
printStep(testname,'Step 2',\
          'STA1 config 4 keys:key5,key6,key3,key7 to connect to network 1,',\
          'connect success.')
res1=res2=1
# operate
res1 = WpaConnectWirelessNetwork(sta1,Netcard_sta1,Network_name1,connectType='custom',
                                key_mgmt='NONE',auth_alg='SHARED',
                                wep_key0=web_key5_ascii,wep_key1=web_key6_ascii,
                                wep_key2=web_key3_ascii,wep_key3=web_key7_ascii,wep_tx_keyidx='2',
