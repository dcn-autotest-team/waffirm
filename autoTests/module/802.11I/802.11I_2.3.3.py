...
#
# Features:
# 2.3.3  WEP加密的Share Key：密钥匹配
# 测试描述：设置wep的4个key分别为key1，key2，key3，key4，设置认证加密使用key1，
#           客户端设置4个key为key5，key1，key6，key7，不能关联上AP
#*******************************************************************************
# Change log:
...
testname = 'TestCase 802.11I_2.3.3'
avoiderror(testname)
printTimer(testname,'Start','Config four keys:key1,key2,key3,key4 in wep share key mode network1 on AC1\n \
                    Sta use four keys:key5,key1,key6,key7 to connect to network1 and connect failed')
...
#Step 2
#操作
#设设置STA1为WEP Share Key认证，设置4各密码分别为key5,key1,key6,key7，关联test1
#
#预期
#关联失败。
################################################################################
printStep(testname,'Step 2',\
          'STA1 config 4 keys:key5,key1,key6,key7 to connect to network 1,',\
          'connect failed.')
res1=1
# operate
res1 = WpaConnectWirelessNetwork(sta1,Netcard_sta1,Network_name1,connectType='custom',
                                ...
                                wep_key0=web_key5_hex,wep_key1=web_key1_hex,
