...
class DcnCommonFtp(object):
    ...
    def upload(self, log_file_list):
        ...
        if self.config['ftp_main_switch']:
            if filename:
                res = [FtpClient(str(_file), self.config['server_dst_folder'],
                                 self.config['ip'], self.config['port'], self.config['username'],
                                 self.config['password'], self.config['ftp_log_name']).file_upload() for _file in
                       filename]
                return res
            else:
    ...
    def upload(self, log_file_list):
        if 'job_id' in self.testlink_args:
            ...
            if isinstance(log_file_list, list):
                ...
                return {'Status': 'fail', 'Message': res}
            if filename:
                res = [FtpClient(str(_file), dst_folder, self.ip, self.port, self.username,
                                 self.password, self.ftp_log_name).file_upload() for _file in filename]
                return res
            else:
                res = '要上传的文件列表为空，请检查配置'
                printResWarn(res)
