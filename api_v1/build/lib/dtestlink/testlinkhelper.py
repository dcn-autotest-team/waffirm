...
#  作者：yanwh
#  修改时间：2017/11/17
#  修改内容：北京testlink服务器已经全部迁移到武汉，因此testlink服务器只存在武汉，因此默认服务器只有192.168.60.60
# -------------------------------------------------------------------------
import os
import re
import subprocess
...
class TestLinkHelper(object):
    ...
    a) TestLink Server URL of XMLRPC
       ...
       default value        - http://192.168.50.193/lib/api/xmlrpc/v1/xmlrpc.php
    ...
    DEFAULT_SERVER_URL = 'http://192.168.60.60/lib/api/xmlrpc/v1/xmlrpc.php'
    DEFAULT_DEVKEY = '292efecf947c37e59ef0b32dbd2b1da1'
    DEFAULT_DESCRIPTION = 'Python XMLRPC client for the TestLink API v%s' \
                          % VERSION
    
    curpath = path = sys.path[0]
    if os.path.isdir(path):
        curpath = path  # python file return path
    if os.path.isfile(path):
        curpath = os.path.dirname(path)  # py2exe file return dir path
    if os.path.isfile(curpath + '\dcnrdc0.log'):
        fp = open(curpath + '\dcnrdc0.log', 'r')
        buf = fp.read()
        fp.close()
        temp = re.search('DEFAULT_SERVER_URL=(.*)', buf)
        if temp:
            DEFAULT_SERVER_URL = temp.group(1)
        else:
            DEFAULT_SERVER_URL = 'http://192.168.60.60/lib/api/xmlrpc/v1/xmlrpc.php'
            fp = open(os.path.join(curpath, 'dcnrdc0.log'), 'a')
            fp.write(time.strftime('%Y-%m-%d %H:%M:%S',
                                   time.localtime(time.time())) + ' :DEFAULT_SERVER_URL=' + DEFAULT_SERVER_URL + '\n')
            fp.close()
