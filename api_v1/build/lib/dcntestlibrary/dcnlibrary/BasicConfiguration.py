...
def WpaConnectWirelessNetworkold(sta, netcardName, ssid, connectType='open', dhcpFlag=True, **args):
    ...
    if connectType.lower() == 'open':
        ...
        if 'wep_key0' in args:
            wep_key0_str = args['wep_key0'] if 'wep_key_type' in args and args[
                                                                                    'wep_key_type'] == 'hex' else '\'"' + \
                                                                                                                  args[
                                                                                                                      'wep_key0'] + '"\''
            SetCmd(sta, 'wpa_cli -i ' + netcardName + ' set_network 0 wep_key0 ' + wep_key0_str)
        if 'wep_key1' in args:
            wep_key1_str = args['wep_key1'] if 'wep_key_type' in args and args[
                                                                                    'wep_key_type'] == 'hex' else '\'"' + \
                                                                                                                  args[
                                                                                                                      'wep_key1'] + '"\''
            SetCmd(sta, 'wpa_cli -i ' + netcardName + ' set_network 0 wep_key1 ' + wep_key1_str)
        if 'wep_key2' in args:
            wep_key2_str = args['wep_key2'] if 'wep_key_type' in args and args[
                                                                                    'wep_key_type'] == 'hex' else '\'"' + \
                                                                                                                  args[
                                                                                                                      'wep_key2'] + '"\''
            SetCmd(sta, 'wpa_cli -i ' + netcardName + ' set_network 0 wep_key2 ' + wep_key2_str)
        if 'wep_key3' in args:
            wep_key3_str = args['wep_key3'] if 'wep_key_type' in args and args[
                                                                                    'wep_key_type'] == 'hex' else '\'"' + \
                                                                                                                  args[
                                                                                                                      'wep_key3'] + '"\''
