...
proc SetIxiaBgpProtocol { args } {
	...
	set LinkFlapDownTime 0
	set LinkFlapUpTime 0
#-add by qidb		2011-6-20
	set OriginProtocol 0
	set NextHopSetMode 1
	set NextHopIpType AddressTypeIpV4
	set NextHopIpAddress 0.0.0.0
	set NextHopMode 1
	set OriginatorId 0.0.0.0
	set EnableClusterList false
	set ClusterList ""
	set EnableAsSegment 0
	set AsList ""
	set AsSegmentType bgpSegmentAsSequence
	set EnableOriginatorId 0
	set OriginatorId 0.0.0.0
	set EnableNextHop true
	set AsPathSetMode 1
	set BgpId [lindex $args [expr [lsearch -exact $args Card] + 1]].[lindex $args [expr [lsearch -exact $args Port] + 1]].0.1
#	set RouterIp 10.1.1.2
#	set GateWay 10.1.1.1
#	set MaskWidth 16
#	set RouterMac 00-00-01-00-00-01
	...
	foreach {para value} [array get arrArgs] {
		...
		switch -exact -- $para {
		   	...
		   	EnableAsSegment {
				set EnableAsSegment $value
			}
			AsSegmentType {
				set AsSegmentType $value
			}
			EnableClusterList {
				set EnableClusterList $value				
			}
			ClusterList {
		   	...
		   	NextHopSetMode {
				set NextHopSetMode $value
		   	}
		   	EnableNextHop {
				set EnableNextHop $value
		   	}
		   	NextHopIpType {
				set NextHopIpType $value
		   	}
		   	EnableOriginatorId  {
				set EnableOriginatorId $value
		   	}
		   	OriginatorId {
				set OriginatorId  $value
		   	}
		   	AsPathSetMode {
				set AsPathSetMode $value
		   	}
		   	BgpId {
				set BgpId $value
		   	}
...
#    	puts "asdfeee"
    	for {set bgpi 0} {$bgpi < $bgprouterangenum} {incr bgpi} {
			...
			if {$EnableAsSegment==true} {
				bgp4AsPathItem setDefault        
				bgp4AsPathItem config -enableAsSegment true
				bgp4AsPathItem config -asList $AsList
				bgp4AsPathItem config -asSegmentType $AsSegmentType
				bgp4RouteItem addASPathItem   
			}
			bgp4RouteItem config -enableLocalPref true
			bgp4RouteItem config -localPref 0
			...
			bgp4RouteItem config -numRoutes $NumOfRoute
			bgp4RouteItem config -iterationStep $Step
			bgp4RouteItem config -originProtocol $OriginProtocol
			bgp4RouteItem config -enableOriginatorId $EnableOriginatorId
			bgp4RouteItem config -originatorId $OriginatorId
			bgp4RouteItem config -enableCluster $EnableClusterList
			bgp4RouteItem config -clusterList $ClusterList
		
			if {$NextHopIpAddress!="0.0.0.0"} {	
				bgp4RouteItem config -enableNextHop true
				bgp4RouteItem config -nextHopIpAddress $NextHopIpAddress
				bgp4RouteItem config -nextHopIpType $IpType
				bgp4RouteItem config -nextHopSetMode bgpRouteNextSetHopManually
			}
...
#    bgp4Neighbor config -asNumMode bgp4AsNumModeFixed
    bgp4Neighbor config -enableBgpId true
    bgp4Neighbor config -bgpId $BgpId
    # And add the neighbor to the server
    bgp4Server addNeighbor neighbor1
	...
	if {[info exist arrArgs(Icmpv6LinkAddress)]} {
       ...
       if {[info exist arrArgs(MstpRecord)]} {
	    ...
	    if {[info exist arrArgs(PppoeTlvType)]} {
          ...
          #
	    #HEX string
	    if {[info exist arrArgs(HEX)]} {
	        set ret [ expr $ret * [ CheckHEXInData $arrArgs(HEX) $data ] ]
	        if {$i == 1} {
	  	        append character "HEX($arrArgs(HEX))"
	        }
	    }
...
#######################################################
#
# CheckHEXInData :检查抓到的流cos是否满足镜像的要求
#
#   args:
#                CheckCaptureStream $chasId $Card1 $Port1 HEX "31 2e 31 2e 31 2e 31"
#                注意字节中间要加空格
#   return: 
#                1 :满足要求
#                0:不满足要求
#
########################################################## 
proc CheckHEXInData { strHEX data }  {
       
	return [regexp -nocase $strHEX $data]
...
proc GetFramesSentRate { chas1 card1 port1 } {
    stat getRate statFramesSent $chas1 $card1 $port1
 	set rate1 [stat cget -framesSent] 
 	PrintRes Print  "The sent frames rate of $chas1 $card1 $port1 is $rate1"
 	return $rate1
...
proc GetFramesReceivedOversize { chas card port } {
	stat get statOversize $chas $card $port
 	set counter [stat cget -oversize] 
 	PrintRes Print "The received oversize frames counter of $chas $card $port is $counter"
 	return $counter
}
